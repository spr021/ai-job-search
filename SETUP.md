...
You'll need API credentials for a model provider - opencode supports many (see the [providers docs](https://opencode.ai/docs/providers/)). This workspace ships configured for a DeepSeek model in `opencode.json`; change the `model` field there to use a different one. See the [opencode docs](https://opencode.ai/docs/) for details.
...
By default, MiKTeX installs missing packages on demand but pops up a GUI prompt for each one — which blocks non-interactive terminals (including opencode's `bash` tool). Turn that into a silent auto-install instead:
...
### Stale `.claude/` files from an older clone
Earlier versions of this repo ran on Claude Code and kept its configuration under `.claude/` (`.claude/settings.json`, and in even older clones a broader `.claude/settings.local.json`). The workspace has since migrated to opencode: the specs live under `.opencode/` and the permission allowlist lives in `opencode.json`. If you cloned before the migration, git may leave the old `.claude/` directory behind in your working copy. It is inert under opencode, but delete it so it cannot be mistaken for live configuration:
