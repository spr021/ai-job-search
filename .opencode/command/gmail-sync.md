...
Confirm the Gmail MCP tools are available (a Gmail MCP server configured under `mcp` in `opencode.json`). If not, tell the user to add and authenticate a Gmail MCP server, then stop - do not attempt this via `bash`, IMAP, or any other channel.
