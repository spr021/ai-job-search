...
1. Check that Notion MCP tools are available in this session (tool names starting with `notion_` or similar). Determine this from the session's own tool list **only** - never by running shell commands that probe MCP servers, which would interrupt the user with a permission prompt before the graceful exit. If the tools are not available, stop and tell the user how to connect:
   > Notion MCP isn't connected. Add the Notion MCP server under `mcp` in `opencode.json` (remote, `https://mcp.notion.com/mcp`), then start a **new session** (servers added mid-session are only picked up on restart), complete its OAuth login, and re-run `/notion-sync`.
2. Verify the connection with one cheap call (e.g. a workspace search). An auth error → tell the user to re-authenticate the Notion MCP server and stop. Never retry in a loop.
3. The Notion MCP server is interactively authenticated, so "connected but not authenticable right now" (expired OAuth, headless/CI context where the login flow cannot run) gets the same graceful exit as "not configured": state the reason in one line and stop. This includes the configured-but-unauthenticated state where the server exposes only its auth handshake and no data tools - **never initiate the OAuth flow from this command and never ask whether to authenticate now**; the one line points at the MCP auth step and the command ends there. Authenticating is the user's move, made outside this command.
...
2. **The posting** - webfetch the job URL and write a readable digest: what the role is, key requirements, practical details (location, deadline, salary if stated). Retry a 403 with browser headers per `.opencode/skills/job-application-assistant/09-web-research.md` first. If the fetch still fails or redirects to a listing page, write "Posting no longer available (checked YYYY-MM-DD)" - **never reconstruct a posting from memory**.
