...
2. **`job_posting.md`** - if it already exists, leave it. Otherwise try webfetch on the tracker row's `source` URL and save the posting text, retrying a 403 with browser headers per `.opencode/skills/job-application-assistant/09-web-research.md`. If the URL is dead (postings expire fast - this is exactly why the archive matters), ask the user to paste the posting, or write a stub noting the posting is unavailable. **Never reconstruct a posting from memory.**
