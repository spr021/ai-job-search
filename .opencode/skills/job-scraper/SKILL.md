...
Run all portal CLI calls in parallel where possible using the `task` tool (`subagent_type: "general"`, one subagent per portal). Collect all `results` arrays into a single pool for Step 2, keeping each result tagged with its source portal skill (for Step 2 `detail` lookups).
...
`.opencode/skills/job-application-assistant/09-web-research.md` before giving up — most
